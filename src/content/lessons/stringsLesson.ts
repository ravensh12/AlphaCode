...
export function generateStrings() {
  return lessonShell(
    ...
    [
      ...
      quizCheckStep(
        'quiz-pattern',
        'In the palindrome check, what happens the moment s[left] != s[right]?',
        'Return False immediately',
        ['Keep scanning to the end', 'Swap the two letters', 'Move only left inward'],
        {
          correct: 'Right — one mismatched pair is enough to rule out a palindrome.',
          incorrect: 'Look at the trace — the code stops as soon as a pair differs.',
          secondIncorrect: 'The if s[left] != s[right] branch returns False right away.',
        },
        ['strings'],
      ),
    ],
